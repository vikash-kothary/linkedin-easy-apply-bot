...
st.set_page_config(
    page_title="LinkedIn Easy Apply Bot", initial_sidebar_state="collapsed"
)
...
with st.form(key="my_form"):
    st.session_state["username"] = st.text_input(
        "Email", value=config.LINKEDIN_USERNAME
    )
    st.session_state["password"] = st.text_input(
        "Password", type="password", value=config.LINKEDIN_PASSWORD
    )
